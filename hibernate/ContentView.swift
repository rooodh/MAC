...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text(isTestMode ? "🔒 Safe Mode: No system changes" : "⚠️ Real Mode: Will change system settings")
                .font(.caption)
        ...
        .sheet(isPresented: $showCommandSheet) {
            CommandInstructionsView(
                ...
                onCopy: {
                    ...
                    addLogMessage("📋 Command copied to clipboard: \\(commandToDisplay)", type: .success)
                },
                onOpenTerminal: {
                    ...
                    addLogMessage("🖥️ Terminal opened (paste command manually)", type: .info)
                },
                onDismiss: {
        ...
        if isTestMode {
            ...
            if success {
                ...
                if success2 && success3 {
                    ...
                    addLogMessage("⚠️ Partial success - some hibernation settings may not be applied", type: .warning)
                    isHibernating = true
        ...
        if isTestMode {
            ...
            if success {
                ...
                if success2 && success3 {
                    ...
                    addLogMessage("⚠️ Partial success - some sleep settings may not be applied", type: .warning)
                    isHibernating = false
    ...
    func executeSudoCommand(_ command: String, successMessage: String) -> Bool {
        ...
        addLogMessage("⚠️ Administrator privileges required", type: .warning)
        
        // Use our helper script to execute sudo commands
        let scriptPath = Bundle.main.path(forResource: "execute_with_sudo", ofType: "sh") ?? ""
        
        if scriptPath.isEmpty {
            addLogMessage("❌ Helper script not found", type: .error)
            alertMessage = "Helper script not found. Please check the application installation."
            showAlert = true
            return false
        }
        
        // Construct the full command to execute
        let fullCommand = "\"\\(scriptPath)\" \\(command)"
        
        // Show instructions to the user
        addLogMessage("💡 Please open Terminal and run this command:", type: .info)
        addLogMessage("   \\(fullCommand)", type: .command)
    ...
    func showCommandInstructions(_ command: String) {
        ...
        addLogMessage("📋 Showing command instructions for: \\(command)", type: .info)
    }
        ...
        do {
            ...
            if let output = String(data: data, encoding: .utf8) {
                if task.terminationStatus != 0 {
                    addLogMessage("❌ Command failed with status \\(task.terminationStatus)", type: .error)
                    addLogMessage("Output: \\(output)", type: .info)
                    return false
                }
                
                addLogMessage("✅ \\(successMessage)", type: .success)
                addLogMessage("Output: \\(output)", type: .info)
                return true
            }
        } catch {
            addLogMessage("❌ Error executing command: \\(error)", type: .error)
            return false
        }
